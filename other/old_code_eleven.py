...
from base_old import main_base
from base_single import main_category_single, add_product_single, get_last_fetch_date, delete_last_record_by_date
CREATED_TABLES = []
CREATED_TABLES_PRODUCT = []
CREATED_TABLES_SINGLE = []
...
def one_single_product(product_data):
    ...
    if title:
        if title not in CREATED_TABLES_SINGLE:
            CREATED_TABLES_SINGLE.append(title)
...
if __name__ == "__main__":
    ...
    print("\n=== Парсинг списка товаров ===")
    category_url = "https://www.ozon.ru/category/golovnye-ubory-zhenskie-17011/?category_was_predicted=true&deny_category_prediction=true&from_global=true&text=шапки+женские+зимние"

    list_html = get_page_preview(category_url, page_type='category')
    if list_html:
        all_products = get_page_all(list_html)
        if all_products:
            main_base(all_products, CREATED_TABLES)
            # Сохраняем в файл
