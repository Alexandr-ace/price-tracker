...
def get_page_all(html):
    ...
    for article in product_articles:
        ...
        book_info['availability'] = avalible_elem.text.strip()
...
def file_csv_funk(csv_file):
    filename = f'books_data_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
    ...
    print(f"Всего сохранено записей: {len(csv_file)}")
